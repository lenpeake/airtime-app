// AirportDetails.js (GPS-integrated with welcome modal, TSA/CLEAR indicators, full table + graph)
...
import { getAirportLogo } from '../components/airportLogos';
import { usePreferredName } from '../components/PreferredNameContext';
...
export default function AirportDetails({ route, navigation }) {
  ...
  return (
    <ScreenWithHeaderFooter navigation={navigation}>
      ...
      <Modal animationType="fade" transparent={true} visible={showWelcome} onRequestClose={() => setShowWelcome(false)}>
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'center', alignItems: 'center' }}>
          <Animatable.View
            animation="fadeInDown"
            duration={600}
            style={{ backgroundColor: '#fff', borderRadius: 16, padding: 24, width: '80%', alignItems: 'center' }}
          >
            <Text style={{ fontSize: 24, fontFamily: 'PlayfairDisplay-Bold', color: '#1f2937', marginBottom: 12 }}>👋 Welcome to</Text>
            <Text style={{ fontSize: 18, fontFamily: 'CormorantGaramond', color: '#4b5563', marginBottom: 20 }}>{airportName} ({airportCode})</Text>
            <TouchableOpacity onPress={() => setShowWelcome(false)} style={{ backgroundColor: '#2563eb', paddingVertical: 10, paddingHorizontal: 20, borderRadius: 8 }}>
