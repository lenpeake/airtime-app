...
export default function AirportDetails({ route, navigation }) {
  ...
  const shownAirportsRef = useRef(new Set());
  ...
  useEffect(() => {
    if (
      isWithinRange &&
      airportCode &&
      !shownAirportsRef.current.has(airportCode)
    ) {
      shownAirportsRef.current.add(airportCode);
      setShowWelcome(true);
  ...
  return (
    <ScreenWithHeaderFooter navigation={navigation}>
      <ScrollView contentContainerStyle={{ paddingBottom: 60, flexGrow: 1 }} nestedScrollEnabled>
        ...
        <Animated.View style={[styles.cardHighlightLux, { transform: [{ scale: pulse }] }]}> 
          <View style={styles.gradientFallbackBox}>
            ...
            <Text style={styles.valueHighlight}>{waitTimes.rightnow_description || t('details.notAvailable')}</Text>
          </View>
        </Animated.View>

        <View style={styles.badgeRow}>
          <View style={[styles.badge, waitTimes.precheck === 1 ? styles.badgeAvailable : styles.badgeUnavailable]}>
            <Text style={styles.badgeText}>TSA Pre✓  {waitTimes.precheck === 1 ? 'Available' : 'Not Available'}</Text>
          </View>
        ...
        <View style={styles.scrollableCard}>
          ...
          <ScrollView style={styles.hourList} showsVerticalScrollIndicator persistentScrollbar nestedScrollEnabled>
            {waitTimes.estimated_hourly_times?.map((slot, index) => (
              <View key={index} style={styles.row}>
                <Text style={styles.time}>{slot.timeslot}</Text>
                <Text style={styles.wait}>
                  {typeof slot.waittime === 'number' ? `${slot.waittime.toFixed(1)} ${t('units.minutes')}` : t('details.notAvailable')}
                </Text>
              </View>
            ))}
          </ScrollView>
      ...
      <Modal animationType="fade" transparent visible={showWelcome} onRequestClose={() => setShowWelcome(false)}>
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'center', alignItems: 'center' }}>
          <Animatable.View animation="fadeInDown" duration={600} style={{ backgroundColor: '#fff', borderRadius: 20, padding: 24, width: '85%', alignItems: 'center' }}>
            ...
            <Image source={getAirportLogo(airportCode)} resizeMode="contain" style={{ width: 220, height: 110, marginBottom: 24 }} />
            <TouchableOpacity onPress={() => setShowWelcome(false)} style={{ backgroundColor: '#2563eb', paddingVertical: 6, paddingHorizontal: 16, borderRadius: 8 }}>
              <Text style={{ color: 'white', fontFamily: 'Inter', fontSize: 16 }}>Continue</Text>
            </TouchableOpacity>
          </Animatable.View>
        </View>
      </Modal>
